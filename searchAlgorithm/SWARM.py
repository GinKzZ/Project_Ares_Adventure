...
def SWARM_solve(matrix, stone_weight):
    ...
    while pq:
        ...
        for dr, dc, move in directions:
            ...
            # Khởi tạo new_stone_pos mặc định là giữ nguyên
            ...
            if new_ares in stone_pos:  # Nếu Ares đẩy một viên đá
                ...
                # Tính trọng số dựa trên trọng số viên đá
            ...
            heuristic = sum(min(manhattan_distance(s, (r, c)) for (r, c) in stone_pos) for s in new_stone_pos)

            if new_state not in visited and matrix[new_ares[0]][new_ares[1]] != "#":
                visited.add(new_state)
                heapq.heappush(pq, (new_cost + heuristic, new_ares, new_state[1], path + move.upper() if new_ares in stone_pos else path + move))
