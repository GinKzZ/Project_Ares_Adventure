...
def BFS_solve(matrix, stone_weight):
    ...
    # Thêm trạng thái ban đầu vào queue
    queue.append((ares, frozenset(stones.keys()), ""))
    ...
    while queue:
        node_count += 1
        (ares_pos, stone_pos, path) = queue.popleft()
        ...
        for dr, dc, move in directions:
            ...
            if new_state not in visited and matrix[new_ares[0]][new_ares[1]] != "#":
                ...
                queue.append((new_ares, new_state[1], path + move.upper() if new_ares in stone_pos else path + move))
